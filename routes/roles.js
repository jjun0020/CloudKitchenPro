...
router.post('/rolesAdd', async function(req,res){
    ...
    try{
        ...
        await newRole.save();
        res.redirect('/api/login-34890645') //after signing up, go to login page
        ...
        return res.status(400).render('add-role', {
            error: errorMessage,
            email,
            fullName,
            role,
...
router.get("/api/add-recipes-34890645", async function (req, res) {
    ...
    res.render('add-recipes', { userId, email, fullName });
});
...
router.post("/addRecipe", async function (req, res) {
    try {
        ...
        res.redirect(`/api/view-recipes-34890645?userId=${req.body.userId}&&fullName=${req.body.fullName}&&email=${req.body.email}`)
    } catch (error) {
        console.log(error);
...
router.get('/api/delete-recipes-34890645', async function (req, res) {
    try {
        ...
        console.log("fullNameForViewRecipe", fullName);
        const recipes = await Recipe.find({});
        res.render('recipe-delete', { recipes, userId, email, fullName });
    } catch (error) {
        console.error(error);
        res.status(500).send('Server Error');
    }
...
router.post('/:id/delete', async (req, res) => {
    try {
        ...
        res.redirect(`/api/view-recipes-34890645?userId=${userId}&&fullName=${fullName}&&email=${email}`);
    } catch (error) {
        console.error(error);
        res.status(500).send('Server Error');
    }
...
router.get('/:id/edit', async (req, res) => {
    try {
        ...
        res.render('edit-recipe', { recipe, userId, email, fullName });
    } catch (error) {
        console.error(error);
        res.status(500).send('Server Error');
    }
...
router.get("/api/add-inventories-34890645", function (req, res) {
    ...
    res.render('add-inventory', { userId, email, fullName });
});
...
router.get('/:id/edit', async (req, res) => {
    try {
        ...
        res.render('edit-inventory', { inventory, userId, email, fullName });
    } catch (error) {
        console.error(error);
        res.status(500).send('Server Error');
    }
...
router.post("/:id/update", async function (req, res) {
    try {
        ...
        res.redirect(`/api/view-inventory-34890645?userId=${aRole.userId}&&email=${aRole.email}&&fullName=${aRole.fullName}`);
    } catch (error) {
        console.error(error);
